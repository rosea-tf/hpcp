...
def run_shear_decay(lat, omega, epsilon):
    ...
    u_x_sin = (epsilon * np.sin(k * lat.y_range))[np.newaxis, :]
    ...
    assert np.allclose(u_sin, u_initial)
...
ax[0].set_ylabel('$y$')
ax[0].set_yticks([])
ax[0].plot(u[0, :, 0], np.arange(Y_FULL_LEN))

ax[1].set_title('Initial streaming pattern in lattice')
ax[1].set_xlabel('$x$')
ax[1].streamplot(np.arange(X_FULL_LEN), np.arange(Y_FULL_LEN), *np.transpose(u_initial_05, [2,1,0]), linewidth=(3/epsilon)*np.linalg.norm(u, axis=2).T)
